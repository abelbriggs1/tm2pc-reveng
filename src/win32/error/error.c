 ...
 * @address        0x004E8840
 */
static const ErrorMsgBox error_table[TM_ERROR_MAX] = {
  {
    RETRY_YES,
    "CD Audio Error",
...
int ErrorMessage (ErrorCode code, ...)
{
  ...
  wvsprintfA (text, error_table[code].errorStrDescription, arglist);
  ShowCursor (TRUE);
  ...
  int result = MessageBoxA (curHwnd, text, error_table[code].errorStrTitle,
                            error_table[code].msgBoxType);
  ShowCursor (FALSE);
